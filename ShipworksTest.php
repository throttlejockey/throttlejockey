...
|
| Copyright 2018 Interapptive, Inc.  All rights reserved.
| http://www.interapptive.com/
|
|
 */
// define('REQUIRE_SECURE', $mode !== "DEV");
define('REQUIRE_SECURE', true); //$mode !== "DEV");


$_REQUEST['username'] = 'chris';
$_REQUEST['password'] = 'Tacobell!23';


// $_REQUEST['action'] = 'getmodule';
// $_REQUEST['action'] = 'getstore';
// $_REQUEST['action'] = 'getcount';
$_REQUEST['action'] = 'getorders';
// $_REQUEST['action'] = 'getstatuscodes';
// $_REQUEST['action'] = 'updatestatus';
...
function checkAdminLogin()
{
	...
	if (isset($_REQUEST['username']) && isset($_REQUEST['password']))
	{
        ...
        // $user_query_str = "SELECT * FROM `" . DB_PREFIX . "user` WHERE username = '" . $db->escape($username) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $db->escape($password) . "'))))) OR password = '" . $db->escape(md5($password)) . "') AND status = '1'";
...
function Action_GetCount()
{
	...
	$query = $db->query("SELECT count(*) AS count FROM `" . DB_PREFIX . "order` WHERE date_modified > '$lastmodified' AND order_status_id != 0")->row['count'];
...
function Action_GetOrders()
{
	...
	//Count of orders newer than $start
	$orders = $db->query("SELECT order_id AS order_id FROM `" . DB_PREFIX . "order` WHERE date_modified > '$lastmodified' AND order_status_id != 0 order by date_modified asc LIMIT 0,50")->rows;
...
function WriteOrderItems($orderItems)
{
	...
	foreach ($orderItems as $item)
	{
		...
		$productSql = "SELECT DISTINCT * FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id ."'";
        
		$product = $db->query($productSql)->rows;
