...
// SELECT * FROM `oc_product` ORDER BY `oc_product`.`product_id` ASC
        ...
        $query = $db->query("
                    SELECT * FROM `oc_category` ORDER BY `oc_category`.`category_id` ASC
                    ");
        // $rows = $query->rows;
print('<pre>');
        ...
        foreach ($query->rows as $result) {
            // print_r ($result);
            $product_data[$result['category_id']] = $result['category_id'];
            $productInfo[$result['category_id']] = getCategory($result['category_id'], $db, $customer, $config);
            // print_r ($productInfo);
        }
            // print_r ($productInfo);
