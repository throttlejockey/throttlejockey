...
// ADDITIONAL SQL TO TARGET SPECIFIC IDs. Quick Hack for Migration on 11.18.21
$CustomerIDs = "8881, 8882, 8883";
$ADDITIONAL_SQL = "WHERE `customer_id` IN ($CustomerIDs)";
$ADDITIONAL_SQL = "";


$cc = 0;
$sql = "SELECT * FROM `" . $db_prefix1 . "customer` $ADDITIONAL_SQL";
$query = mysqli_query($con1,$sql);
...
while($result = $query->fetch_assoc())
{
	...
	$safe             = true; //(empty($result['safe'])) ? 1 : addslashes($result['safe']);
	$token            = $result['token'];
	$code             = '';
