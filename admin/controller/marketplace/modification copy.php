    ...
    public function recursiveRemoveDirectory($directory)
    {
        ...
        foreach(glob("{$directory}/*") as $file)
        {
            if(is_dir($file)) {
                ...
                // $this->log->write("Dir: $file");
            } else if(!is_link($file)) {
                // $this->log->write("File: $file");
                unlink($file);
            }
        }
        // $this->log->write("rmdir('$directory')"); //: $file");
        rmdir($directory);
    }
...
//karapuz (Ka Extensions platform)
        if (!$this->user->hasPermission('access', 'extension/extension/ka_extensions')) {
            $this->load->model('user/user_group');
            $this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/extension/ka_extensions');
            $this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/extension/ka_extensions');
        }
///karapuz (Ka Extensions platform)
//karapuz (Ka Extensions platform)
        if (!$this->user->hasPermission('access', 'extension/extension/ka_extensions')) {
