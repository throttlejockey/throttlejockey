		...
		methods: {
			...
			update: function (json, confirm) {
				if (json.response.redirect) {
                    ...
                    this.raw_json = json.response;

                    console.log(" FIX THE SORTING BY COST IN Checkout.js !! ");
                    console.log(JSON.stringify(this.shipping_methods, null, 4));
                    console.log(JSON.parse(JSON.stringify(this.shipping_methods)));

                    console.log( Object.keys(this.shipping_methods) );
                    for( shipping_method in this.shipping_methods) {
