...
class ModelExtensionShippinghitshippodhlexpress extends Model {
	function getQuote($address) {
		...
		if ($status) {
			...
			foreach($products as $sing_product)
			{
				if(isset($sing_product['shipping']) && $sing_product['shipping'] == 1)
				{
                    ...
                    // $this->log->write('--- DHL SING PRODUCT Request ---');
                    // $this->log->write( var_export($sing_product, 1) );

                    if( $sing_product['weight_class_id'] == '5' ) {
                        // $this->log->write( "Weight Class is LBS!"); // var_export($sing_product, 1) );
                    }
                    if( $sing_product['weight_class_id'] == '6' ) {
                        $this->log->write( "Converting Oz to LBS!"); // var_export($sing_product, 1) );
                        $sing_product['weight'] = $sing_product['weight'] / 16; 
                        $sing_product['weight_class_id'] = '5'; 
                    }
            ...
            // I only want the largest package out of the array.. 
            $pieces = $this->hitshippo_get_largest_package($dhl_packs);
            ...
            $this->log->write('--- DHL Request ---');
            // $this->log->write( var_export(htmlspecialchars($request), 1) );
            $this->log->write( print_r($request, 1) );
        ...
        // ChromePhp::log( print_r($address, 1 ) );
            ...
            $hasRemoteFee = ""; // assume no remote fee initially.
			...
			if ($result && !empty($result->GetQuoteResponse->BkgDetails->QtdShp)) {
				...
				foreach ($result->GetQuoteResponse->BkgDetails->QtdShp as $quote) {
                    ...
                    // $this->log->write(  "Rate: [$rate_code] $rate_title <br>");
                    // $this->log->write(  "Transportation Charges: $rate_charge");
                    // $this->log->write(  "Discount Applied: -$discount");
 
 


                    // $this->log->write( "============" );
                    // $this->log->write( "rate_cost: " );
                    // $this->log->write( $rate_cost );

                    if( !empty($quote->QtdShpExChrg) ){

                        // $this->log->write('Extra Charges:');
                        // $this->log->write('---');
                        foreach($quote->QtdShpExChrg as $ExtraCharge) {
                            ...
                            // ChromePhp::log("-- DHL :: Test Log. --");
                            if (strpos($exName, 'REMOTE') !== false) {
                                // ChromePhp::log("-- DHL :: Remote Fee! --");
                                // $rate_title .= " [remote fee]";
                                // $title .= " [remote fee]";
                                $hasRemoteFee = " [remote fee]";
                            }

                            $rate_cost_all = ( float )$rate_cost_all + ( float )$exCost;
        
                            // $this->log->write( "$exName: $exCost");
                    ...
                    // $rate_cost = $rate_cost_all;
                    
                    // $this->log->write( 'Total Cost: ' . (float)$rate_cost_all );


					$selected_services_aaray = $this->config->get('shipping_hitshippo_dhlexpress_service');

                    // $this->log->write(print_r($selected_services_aaray, 1));
...
//echo "<pre>";		//	print_r($result);		//	die();
    ...
    function hitshippo_get_largest_package( $dhl_packages ) {
        $pieces = '';
        $dimensionString = '';
        $largestSize = 0;
        $pieceID = -1;
        ...
        // ChromePhp::log("-- DHL  --");
        // ChromePhp::log( print_r($dhl_packages, 1 ) );
        // $this->log->write("hitshippo_get_largest_package.");
        // $this->log->write( print_r($dhl_packages, 1) );

        if ( $dhl_packages ) {
            
            $pieces .= '<Piece><PieceID>1</PieceID>';
            $pack_type = "BOX"; //$this->hitshippo_get_pack_type( $parcel['packtype'] );
            ...
            foreach ( $dhl_packages as $key => $parcel ) {
                ...
                if ( !empty( $parcel['Dimensions']['Height'] ) && !empty( $parcel['Dimensions']['Length'] ) && !empty( $parcel['Dimensions']['Width'] ) ) {
                    ...
                    if ( $currentSize > $largestSize ) {
                        ...
                        $dimensionString  = '<Height>' . $parcel['Dimensions']['Height'] . '</Height>';
                        $dimensionString .= '<Depth>' . $parcel['Dimensions']['Length'] . '</Depth>';
                        $dimensionString .= '<Width>' . $parcel['Dimensions']['Width'] . '</Width>';
                    }
    
                }
            }

            $pieces .= $dimensionString;
